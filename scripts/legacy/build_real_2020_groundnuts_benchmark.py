#!/usr/bin/env python3
"""
Build and evaluate the 2020 real-data benchmark for Eastern Uganda groundnuts
yield using AAS 2020 real yield assignments.

This benchmark is intentionally narrow:
  - year: 2020 only
  - geography: Mbale, Kapchorwa, Iganga, Jinja, Tororo
  - target: real AAS 2020 groundnuts yield assigned from sub-region totals

Outputs:
  - eastern_uganda_groundnuts_real_benchmark_2020.csv
  - real_2020_groundnuts_benchmark_model_results.csv
  - real_2020_groundnuts_benchmark_pca_loadings.csv
  - real_2020_groundnuts_benchmark_pca_explained_variance.csv
  - real_2020_groundnuts_benchmark_summary.txt
"""
...
YIELD_FILE = Path("aas2020_eastern_district_groundnuts_yield_total_2020.csv")
BENCHMARK_FILE = Path("eastern_uganda_groundnuts_real_benchmark_2020.csv")
RESULTS_FILE = Path("real_2020_groundnuts_benchmark_model_results.csv")
LOADINGS_FILE = Path("real_2020_groundnuts_benchmark_pca_loadings.csv")
VARIANCE_FILE = Path("real_2020_groundnuts_benchmark_pca_explained_variance.csv")
SUMMARY_FILE = Path("real_2020_groundnuts_benchmark_summary.txt")

TARGET = "yield_tons_ha"
BENCHMARK_LABEL = "Eastern Uganda groundnuts real-data benchmark, 2020 only"
BENCHMARK_FEATURES = [
    "MAM",
    "SON",
    "annual_rainfall",
...
def load_benchmark_dataset():
    ...
    yield_df = yield_df.rename(
        columns={
            "yield_tons_ha": "yield_tons_ha_groundnuts2020",
            "yield_tons_ha_planted": "yield_tons_ha_planted_groundnuts2020",
            "area_planted_ha": "area_planted_ha_groundnuts2020",
            "area_harvested_ha": "area_harvested_ha_groundnuts2020",
            "production_mt": "production_mt_groundnuts2020",
            "cv_area_planted_pct": "cv_area_planted_pct_groundnuts2020",
            "cv_area_harvested_pct": "cv_area_harvested_pct_groundnuts2020",
            "cv_production_pct": "cv_production_pct_groundnuts2020",
        }
    )

    merged = features.merge(yield_df, on=["district", "year"], how="inner", validate="one_to_one")
    if merged.empty:
        raise ValueError("No merged 2020 groundnuts benchmark rows were produced.")

    merged["yield_tons_ha_original_maize_proxy"] = merged["yield_tons_ha"]
    merged["yield_tons_ha"] = merged["yield_tons_ha_groundnuts2020"]
...
def main():
    print("=" * 70)
    print("  BUILD 2020-ONLY REAL-DATA GROUNDNUTS BENCHMARK")
    print("=" * 70)
