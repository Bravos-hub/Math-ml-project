...
def test_loso_is_spatially_disjoint():
    ...
    for train, test in leave_one_subregion_out_splits(frame):
        assert set(frame.iloc[train].spatial_unit).isdisjoint(
            frame.iloc[test].spatial_unit
        )
...
def test_crop_baseline_is_training_only_and_informative_under_spatial_holdout():
    ...
    crop_output = result["training_crop_mean"]
    assert not crop_output.fallback_used.any()
    assert crop_output.values.tolist() == [1.0, 10.0]
